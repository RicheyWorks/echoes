...
- Single `Agent` with a clear goal
- **Cryptographic memory integrity** using SHA-256 hash chaining
- Append-only audit log
- Immutable goal after agent creation
- `verify_integrity()` method to detect tampering
- `print_memory_chain()` and `print_audit_log()` helpers
- Comprehensive **unit tests** (`cargo test`) covering integrity, tamper detection, and chaining
- Built-in educational **tamper detection demo** in `main()`
...
You should see the agent taking actions, pretty-printed chain + audit log, integrity check, and the tamper demo.
