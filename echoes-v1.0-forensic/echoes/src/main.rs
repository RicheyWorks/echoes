...
/// Structured security events for forensic logging
...
pub struct MemoryEntry {
    pub tick: u32,
    pub action: Action,
    pub event: SecurityEvent,   // Structured forensic event
    pub note: String,           // Additional context
...
impl Agent {
    ...
    fn compute_hash(prev_hash: &[u8; 32], tick: u32, action: &Action, event: &SecurityEvent, note: &str) -> [u8; 32] {
        let mut hasher = Sha256::new();
    ...
    pub fn think(&mut self) {
        ...
        // Create a structured security event (forensic logging)
        let event = match &action {
            Action::Observe => SecurityEvent::Custom("environment scan performed".to_string()),
            Action::Explore => SecurityEvent::Custom(format!("exploring toward goal")),
    ...
    pub fn verify_integrity(&self) -> bool {
        ...
        for entry in &self.memory {
            ...
            let recomputed =
                Self::compute_hash(&entry.prev_hash, entry.tick, &entry.action, &entry.event, &entry.note);
...
fn main() {
    println!("=== echoes v1.0 (Forensic Security Agent - Structured Events + Merkle + Hash Chain) ===\n");
